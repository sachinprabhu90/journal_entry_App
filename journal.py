...
class Journal():
    def __init__(self, master):
        ...
        style = ttk.Style()
        style.configure('TEntry', foreground='blue', background='grey', font=('Arial', 20))
        style.configure('TButton', foreground='blue', background='LightBlue3', font=('Arial', 12))
        style.configure('TLabel', foreground='blue', font=('Arial', 12))

        ttk.Label(master, text='Date & Time: ').grid(
            row=0, column=0, padx=2, pady=3)
        self.datetime_entry = ttk.Entry(master, state=DISABLED, style='entry.TEntry')
        ...
        self.refresh_location_button = ttk.Button(master, text='Refresh', command = lambda: self.refresh_location())
        ...
        self.text_entry = Text(master, height=30, foreground='blue' ,font=('Time New Roman', 15))
    ...
    def refresh(self):
        ...
        self.datetime_entry.insert(0,datetime.now().strftime('%H:%M:%S %d-%m-%Y'))
    ...
    def save_to_file(self):
        self.filename = filedialog.askopenfilename(filetypes = (("Text File", "*.txt"),) )
        ...
        if isinstance(self.filename, str) and os.path.isfile(self.filename):
            ...
            messagebox.showinfo(title='Info', message=f'saved to {self.filename}')
        else:
            pass
